...
_u.extend(Injector.prototype, {
    ...
    resolve: function invoke$resolve(fn, scope, ctx) {
        ...
        when(resolvers.map(function (f) {
            return f.call(api, ctx)
        })).then(function (args) {
            try {
                wp.resolve(null, fn.apply(scope, args));
            } catch (e) {
                wp.reject(e);
            }
//            Function.apply.bind(fn, scope)
        }, wp.reject);
    ...
    resolveBind: function invoke$resolveBind(fn, scope) {
        ...
        return function invoke$resolveBind$return(ctx) {
            ...
            var p = promise();
            whenCB(resolvers.map(function (f) {
                return f.call(api, ctx);
            })).then(function (args) {
                try {
                    p.resolve(null, fn.apply(scope, args));
                } catch (e) {
                    console.trace(e);
                    p.reject(e);
                }
