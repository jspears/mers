...
function onEachResolverMap(v) {
    var parts = v.split('$'), resolvers = parts.length > 1 ? [parts.shift()] : api._resolveResolvers(this, v);
    return api.resolveWrap(this, resolvers, parts);
}
var resolver = {

}
function Resolver(defSettings){
    _u.extend(this, defSettings
...
var api = {
    ...
    _resolveResolvers: function (settings, v) {
        return settings.resolverMap[v] || settings.search;
    },
    ...
    resolve: function () {
        return api['_' + defSettings.resolverStyle].apply(api, arguments);
    },
    ...
    _resolve: function invoke$resolve(fn, scope, ctx) {
        ...
        return fn.apply(scope, api.extractResolvers(fn).map(function invoke$resolve$map(f) {
...
//            return cb(new Error("Object is null for path [" + str + "]"))
        ...
        if (typeof obj[current] === 'function') {
            return api.invoke(api.resolve(obj[current], obj, ctx), str, cb, ctx);
        }
        ...
        if (api.isExec(obj)) {
            return obj.exec(function (e, o) {
                if (e) return cb(e);
                api.invoke(o, str, cb, ctx);
            });
        }
        if (obj instanceof Function) {
            try {
                resp = api.resolve(obj, ctx);
