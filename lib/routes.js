...
module.exports = function MongooseRestRouter(base, router, options) {
    ...
    router.get(reGet, function (req, res, next) {
        ...
        if (params.length) {
            ...
            find.exec(function (err, obj) {
                ...
                invoker.invoke(obj, params, _u.pick(req, 'query', 'session', 'params', 'body'), cbs.asCallback());
            });
        } else {
    ...
    router.put(reGet, function (req, res, next) {
        ...
        var saveTo;
        invoker.invoke(Model.findOne(idObj(params.shift())), params, _u.pick(req, 'query', 'session', 'params', 'body'), function put$advice(str, ctx, obj, advice, nValue) {
            //Can't save embedded documents, so we will do this.
            if (nValue && nValue.constructor.name === 'model') {
                saveTo = nValue;
            }
            if (str.length === 0) {
                nValue.set(ctx.body);
